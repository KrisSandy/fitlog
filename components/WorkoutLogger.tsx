...
import { days, type Day } from "@/lib/routineData";
import {
  saveWorkoutLog, getLogForDate, getTodayDateString, parseRepsToSets,
  type LoggedExercise, type LoggedSet, type WorkoutLog,
} from "@/lib/storage";
...
function getRoutineForDate(dateStr: string): Day {
  const dayIndex = new Date(dateStr + "T00:00:00").getDay();
  const dayId = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"][dayIndex];
  return days.find(d => d.id === dayId)!;
}

/** Build a fresh WorkoutLog from the routine for a given date */
function buildLogFromRoutine(dateStr: string): WorkoutLog {
  const routineDay = getRoutineForDate(dateStr);
  return {
    id: `workout-${dateStr}`,
    date: dateStr,
    dayType: routineDay.type,
...
export default function WorkoutLogger() {
  ...
  const loadDate = (dateStr: string) => {
    ...
    if (existing) {
      ...
      const fresh = buildLogFromRoutine(dateStr);
      setLog(fresh);
      setNotes("");
    }
  };
  ...
  const toggleSet = (exIdx: number, setIdx: number) => {
    if (!log) return;
    const updated = { ...log, exercises: log.exercises.map((ex, ei) =>
  ...
  const routineDay = getRoutineForDate(currentDate);
  const dayColor = routineDay.color;
  const dayGlow = routineDay.glow;
  const isRestDay = routineDay.type === "rest";

  // Stats
  const totalSets = log.exercises.reduce((acc, ex) => acc + ex.sets.length, 0);
  const completedSets = log.exercises.reduce((acc, ex) => acc + ex.sets.filter(s => s.completed).length, 0);
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "center", gap: 12 }}>
          <div style={{
            ...
            background: isRestDay ? "#13131e" : `${dayColor}15`,
            border: `1px solid ${isRestDay ? "#1e1e2e" : `${dayColor}40`}`,
            color: isRestDay ? "#6b7280" : dayColor,
            padding: "8px 16px",
            borderRadius: 20,
            fontSize: 14,
            fontWeight: 600,
            ...
            {isRestDay ? "REST DAY" : `${routineDay.name.toUpperCase()} DAY`}
          </div>
          {totalSets > 0 && (
        ...
        {/* Rest day message */}
        {isRestDay && (
          <div style={{ marginBottom: 24, background: "#0f0f18", border: "1px solid #1e1e2e", borderRadius: 16, padding: 20, textAlign: "center", color: "#6b7280", fontSize: 14, lineHeight: 1.6 }}>
            Rest days are essential. Focus on recovery, hydration, and sleep. You can still log activities below.
          </div>
        )}
                  ...
                  <div key={setIdx} style={{
                    ...
                    <input
                      ...
                      style={{
                        ...
                        borderColor: set.completed ? "#22c55e30" : "#2a2a3a",
                      }}
                    />
        ...
        {log.exercises.length === 0 && (
          <div style={{ marginBottom: 24, background: "#0f0f18", border: "1px solid #1e1e2e", borderRadius: 16, padding: 20, textAlign: "center", color: "#6b7280", fontSize: 14 }}>
            No exercises for this day. Add one below to get started.
          </div>
        )}
          ...
          <div style={{ marginBottom: 24, background: "#0f0f18", border: "1px solid #1e1e2e", borderRadius: 14, padding: 16 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={addCustomExercise} style={{ flex: 1, background: dayColor, border: "none", borderRadius: 8, padding: "10px 12px", color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "inherit" }}>
                  Add
                </button>
                <button onClick={() => setShowAddExercise(false)} style={{ flex: 1, background: "#13131e", border: "1px solid #2a2a3a", borderRadius: 8, padding: "10px 12px", color: "#6b7280", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "inherit" }}>
                  Cancel
                </button>
              </div>
            </div>
